...
現在は **Prototype 0.1の初期実装段階** です。Phase 1の一人称3D操作は実装とWindows実機確認まで完了しました。Phase 2では採掘可能な岩、耐久値、左クリック採掘、成功/空振りFeedback、岩破壊、代表鉱石Dropまで実装し、Windows実機で採掘ループを確認する段階です。
...
現在のテストシーンでは、WASD移動・マウス視点操作・衝突・Escによるカーソル切替に加え、中央の岩へ照準を合わせて左クリックすると採掘できます。岩は3回で壊れ、代表鉱石がDropするPrototypeです。
